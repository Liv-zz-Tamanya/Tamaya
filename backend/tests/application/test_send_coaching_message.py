...
async def test_signal_extraction_failure_does_not_break_reply():
    ...
    uc = SendCoachingMessageUseCase(CoachingAgent(ai), extract)
    reply = await uc.execute(device_id="dev-1", message="오늘 힘들었어", history=[])
    assert reply == "괜찮아요"
    assert extract.calls[0]["device_id"] == "dev-1"


async def test_authenticated_device_id_is_passed_to_signal_extraction():
    ai = _FakeCoachAi("괜찮아요")
    extract = _SpyExtract()
    uc = SendCoachingMessageUseCase(CoachingAgent(ai), extract)

    reply = await uc.execute(device_id="device-A", message="오늘 힘들었어", history=[])
...
async def test_blank_device_id_is_rejected():
    ...
    with pytest.raises(ValueError, match="device_id is required"):
        await _uc(ai).execute(device_id="   ", message="오늘 힘들었어", history=[])
