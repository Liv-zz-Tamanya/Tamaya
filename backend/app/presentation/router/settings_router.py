...
device_id 키잉(User 테이블 없음) — 값은 Bearer 토큰 세션에서 추출한다(타 계정 설정
조회·조작 방지). /test는 계정 데이터 접근이 없어 인증을 요구하지 않는다.
"""
...
async def put_clova_setting(
    ...
    await repo.upsert(ClovaSetting(device_id=device_id, masked_key=masked, has_key=True))
