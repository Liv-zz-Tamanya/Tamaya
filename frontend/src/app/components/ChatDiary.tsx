  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-[#FFF5E6] to-[#FAF8F5]">
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-[#E8DCC8] p-4 flex items-center justify-between">
        ...
        {/* TTS 버튼 — Phase 2 음성 기능 비활성화 (hidden 유지) */}
        <button
          onClick={() => setIsTTSEnabled(!isTTSEnabled)}
          className="hidden p-2.5 rounded-full hover:bg-[#F5E6D3] transition-all"
          aria-hidden="true"
          tabIndex={-1}
        >
          {isTTSEnabled ? (
      ...
      <div className="p-4 bg-white/80 backdrop-blur-sm border-t border-[#E8DCC8]">
        {/* 서브카피 — 텍스트 우선 안내 */}
        <p className="text-xs text-[#A0937D] text-center mb-2 font-diary">
          ✍️ 텍스트로 먼저 시작해요 — 음성 기능은 곧 추가됩니다
        </p>
        <div className="flex items-center gap-2">
          {/* 마이크 버튼 — Phase 2 음성 기능 비활성화 (hidden 유지) */}
          <button
            onClick={isRecording ? stopRecording : startRecording}
            className={`hidden p-3.5 rounded-full transition-all ${
              isRecording
          ...
          <input
            ...
            placeholder="이음이에게 오늘 하루를 이야기해 주세요..."
