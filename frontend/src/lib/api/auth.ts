...
function newAnonDeviceId(): string {
 ...
 *  익명 전용 — "nick-*" 네임스페이스는 BE가 400으로 거부하므로(비밀번호 우회 차단),
 *  로그아웃 등으로 남은 닉네임 device_id는 새 익명 id로 교체한다. */
export async function ensureDeviceToken(): Promise<string> {
  const existing = getToken();
  if (existing) return existing;
  let device_id = getDeviceId();
  if (device_id.startsWith('nick-')) {
    device_id = newAnonDeviceId();
    try {
...
/** 닉네임+비밀번호 회원가입. 이미 있으면 ApiError(409), 비밀번호 정책 위반 400. */
export function signupWithNickname(
  nickname: string,
  password: string,
): Promise<{ isNew: boolean }> {
  return nicknameAuth('/auth/nickname/signup', nickname, password);
}

/** 닉네임+비밀번호 로그인. 없는 닉네임 404, 비밀번호 불일치 401. */
export function loginWithNickname(
  nickname: string,
  password: string,
): Promise<{ isNew: boolean }> {
