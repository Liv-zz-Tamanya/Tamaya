...
- `input_guardrail`: HEALTH mode의 최신 `HumanMessage`만 `medical_guardrail`로 분류한다.
  DIARY mode는 pass-through로 `SAFE` 처리한다.
...
- `tools`: LangGraph `ToolNode`에 등록된 읽기 전용 Tool을 실행한다. Tool 실행 로직을
  Agent가 직접 구현하지 않는다.
- `output_guardrail`: HEALTH mode의 최종 `AIMessage`에 처방 용량·복용 지시 표현이 섞였는지
  `contains_prescriptive_content()`로 검사한다. 감지되면 동일 message id로
...
Health prompt의 진단·처방 금지 문구는 모델 생성 보조 정책이다. 입력 차단과 출력 치환은 Graph
Node의 결정론 정책이 담당한다.
...
HEALTH mode의 위험 입력은 정상 assistant message로 저장되는 고정 면책 응답이다. 출력
tripwire가 동작한 경우 unsafe 모델 응답은 최종 반환값과 Domain `HealthMessage`에 저장되지
않고, 동일 message id의 면책 `AIMessage`로 교체된다.
...
다음 단계에서 Coaching route의 `PersonalAssistantAgent` 전환, 기존 `ChatAgent`/`HealthChatAgent`
cleanup, Tool/model timeout, 오류 분류와 선택적 retry, LangGraph checkpointer/resume,
